...
class SessionSplitterTest {
    ...
    fun sameDaySession_isNotSplit() {
        ...
        val result = SessionSplitter.split(
            start.timeInMillis, end.timeInMillis
        )

        assertEquals(1, result.size)
        assertEquals(30 * 60 * 1000L, result[0].durationMs)
    }
    ...
    fun sessionCrossingMidnight_isSplit() {
        ...
        assertEquals(10 * 60 * 1000L, result[0].durationMs)
        assertEquals(10 * 60 * 1000L, result[1].durationMs)
    }
}